...
```{r setup, include=FALSE}
    library(reticulate)
      use_python("C:/Program Files/Python311/python.exe", required = TRUE)
    library(tensorflow)
    library(keras3)
...
**My Answer:** Since we are only performing exploratory modeling and sampling a large dataset, missing values are rare or encoded as specific numeric codes like we discussed in class (like 9999999). That's why we only need to remove invalid income values because they directly affect the target variable. For tree-based models like XGboost, they can internally handle the missing values because they split at those values. If the value is missing, it will perform a different function that if the value is not missing.
...
**My Code:** Step 1: import the required packages
...
Step 2: Filtering out nonpositive income values:

```{python}
df_sample = df_sample[df_sample["inctot"] > 0]
```

Step 3: Creating the log-transformed income column
...
Step 4: Creating the Histograms:
...
**My Answer:** The raw distribution is heavily right skewed, which reflects reality but is harder for neural networks to understand and process, so the logarithmic transformation allows the distribution to look more like a normal distribution and therefore makes it easier for the data to be process-able. We are focusing on percentage errors, a more meaningful result will be the percentage differences rather than the direct \$ differences which are quite stark and hard to work with in terms of data.

## Task 2: Feature Engineering and Encoding

Split the data into a feature matrix `X` and target vector `y` (the log-income column created above). Perform one-hot encoding on all categorical features, dropping sparse columns whose proportion of non-zero entries falls below 1%. The feature matrix `X` should include all columns except `inctot` and `log_inctot`. The target vector `y` should be the `log_inctot` column. Mimic the one-hot encoding workflow from the tutorial: build dummies for every categorical column in `X`, then drop sparse columns whose proportion of non-zero entries falls below 1%. Step One: label the target vector and feature matrix as such with the following code.
...
Step Six: Verify we edited the data correctly.
...
Reproduce the final modeling attempt from `examples/XGBoost.ipynb`, but treat the log-income target as the label. Work through the following subtasks and insert your own code in each block. **My Code:**

```{python}
import xgboost as xgb
from sklearn.metrics import mean_squared_error, r2_score
print("XGBoost and metrics libraries loaded successfully!")
```

### Task 3.1: Build DMatrix Objects

Create `xgb.DMatrix` objects for both the training and test splits using the log-income labels. Prepare data for XGBoost by converting the pandas DataFrames into numeric arrays (NumPy arrays)
...
Start from the tutorial’s baseline settings (e.g., `eta = 1`, `max_depth = 10`, `min_child_weight = 100`, RMSE metric, early stopping after 50 rounds) and justify any deviations you make. Use the `reg:squarederror` objective and `rmse` evaluation metric.
...
```{python}
  best_params = {
    ...
    print("Best parameters selected from grid search:")
    for k, v in best_params.items():
    print(f"  {k}: {v}")
```

This final configuration reflects a model that learns slowly but deeply, which is well‑suited to the complexity of income data. The small learning rate (eta = 0.01) ensures that each boosting round makes only minor adjustments, reducing the risk of overfitting. The deeper trees (max_depth = 10) allow the model to capture nuanced relationships between demographic, occupational, and educational features that influence income. Together, these settings balance flexibility and stability, producing the lowest validation RMSE observed in the tuning process. 

## Task 3.5: Evaluate Predictions Plot predicted vs. actual log-income values on the test set using a scatter plot.
...
**Step One:** Create DMatrix for test data

```{python}
dtest = xgb.DMatrix(X_test)
```

**Step Two:** Generate predictions

```{python}
y_pred = model.predict(dtest)
```

**Step Three:** Combine actual and predicted values into a DataFrame
...
**Step Four:** Plot predicted vs. actual log‑income

```{python}
    plt.figure(figsize=(7, 7))
    sns.scatterplot(x="Actual", y="Predicted", data=pred_df, alpha=0.5)
    max_val = max(pred_df.max())
    min_val = min(pred_df.min())
      plt.plot([min_val, max_val], [min_val, max_val], color="pink", linestyle="--")

    plt.title("Predicted vs. Actual Log-Income (Test Set)")
...
**My Answer:** The scatterplot shows that the model’s predictions generally align well with the actual log‑income values, but the strength of that alignment varies across the income range. At lower actual log‑income values, the predicted log‑income values are more widely dispersed. This suggests that the model has greater uncertainty or less precision when predicting incomes at the lower end of the scale. In contrast, for mid‑ to high‑income ranges, the points are tightly clustered along the 45‑degree line, indicating that the model captures these relationships much more accurately. The test RMSE of 0.8074 supports this overall good fit, showing that the average prediction error is modest on the log‑income scale. In summary, the model performs well for most individuals, particularly in the middle and upper income ranges, but shows more variability when estimating lower incomes, which is unsurprising.

## Task 4: Neural Network Modeling (Keras)

Use the structure from `examples/neural_nets_simple.ipynb` to fit a fully connected neural network on the same encoded features and log-income target. Use the same train/test split as in Task 2.1.

### Task 4.1: Prepare Data for Keras

If necessary, convert the training and test feature matrices and target vectors into NumPy arrays suitable for Keras. **Note:** You may have already done this in Task 2.1.
...
print("Training features shape:", X_train_nn.shape)
print("Test features shape:", X_test_nn.shape)
print("Training target shape:", y_train_nn.shape)
print("Test target shape:", y_test_nn.shape)
```
...
```
**Step Two:** Determine the number of input features
```{python}
input_dim = X_train_nn.shape[1]
print("Input dimension:", input_dim)
```
**Step Three:** Define a dense neural network with ReLU activations
```{python}
nn_model = Sequential([
...
print("Congratulations, your network is successfully defined!")
nn_model.summary()
```
## Question 5: What should the input dimension be for the first layer of the network?
**My Answer:** The input dimension for the first layer must equal the number of features in the training dataset after all preprocessing and one-hot encoding. In this case, the encoded feature matrix X_train has 693 columns, so the input layer must have input_dim = 693. This allows the network to accept each observation as a 693-dimensional feature vector.
```{python}
print("Number of input features for the first layer:", input_dim)
print("Model input shape:", nn_model.input_shape)
print("First layer configuration:", nn_model.layers[0].get_config())
```
...
**Step One:** Ensure all Data is Numeric. 
```{python}
